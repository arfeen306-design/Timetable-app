...
export function listSubjects(projectId: number) {
  return api<{ id: number; name: string; code: string; color: string; category: string }[]>(`/api/projects/${projectId}/subjects`);
}

export function createSubject(projectId: number, data: { name: string; code?: string; color?: string; category?: string }) {
  return api<{ id: number }>(`/api/projects/${projectId}/subjects`, { method: "POST", body: JSON.stringify(data) });
}

export function updateSubject(projectId: number, id: number, data: Partial<{ name: string; code: string; color: string }>) {
  return api(`/api/projects/${projectId}/subjects/${id}`, { method: "PATCH", body: JSON.stringify(data) });
}
...
export function listRooms(projectId: number) {
  return api<{ id: number; name: string; code: string; room_type: string; capacity: number }[]>(`/api/projects/${projectId}/rooms`);
}
